...
public class MainController {
	...
	public ResponseEntity<String> updateEvent(@RequestBody Event event) {
		return ResponseEntity.ok(eventService.updateEvent(event) ? "Event was successfully updated"
				: "Event was not found and could not be updated");
	}
	...
	public ResponseEntity<String> bookTicket(@RequestBody EventBookDTO eventBook) {
		return ResponseEntity.ok(eventService.bookTicket(eventBook)
				? String.format("Ticket was successfully booked for holder %s and event %s",
						eventBook.getTicketHolder(), eventBook.getEventName())
				: String.format("Error booking ticket for holder %s and event %s", eventBook.getTicketHolder(),
						eventBook.getEventName()));
	}
